...
local function update_extmark(opts, position, bufnr)
	...
	local extmark_opts = {
		...
		virt_text_pos = (virt_text_enabled and type(opts.virtual_text.pos) == "string") and opts.virtual_text.pos
			or nil,
		...
		for client_id, resp in pairs(responses) do
			if not opts.ignore_id[client_id] and resp.result and not vim.tbl_isempty(resp.result) then
				if opts.ignore.actions_without_kind then
					for _, r in pairs(resp.result) do
						for _, i in pairs(opts.ignore.actions) do
							if r.kind and r.kind ~= "" and r.title ~= i then
								has_actions = true
							end
							break
						end
				...
				for _, r in pairs(resp.result) do
					...
					if
						(not opts.ignore.actions_without_kind or has_kind)
						and function()
							canpass = true
							for _, i in pairs(opts.ignore.actions) do
								if r.title == i then
